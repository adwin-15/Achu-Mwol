...
if (conf.LANG == 'EN') vtalk_dsc = 'Achu starts her voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Achu') && conf.ACHU_AI !== 'true') {        
        ...
        var finm = message.message.replace('Achu', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.ACHU_AI == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Achu features. Turn your account into a ai chatbot!'
    already_on = 'Achu artificial intelligence is already fully functional.'
    already_off = 'Achu artificial intelligence is currently running semi-functional.'
    succ_on = 'Achu Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Achu Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ pinky സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'അച്ചു കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'അച്ചു AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'അച്ചു പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി അച്ചു സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}
